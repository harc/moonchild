var whizzy = (function() {
  ...
  function connect(el, TextModel) {
    ...
    doc.addEventListener('selectionchange', function() {
      ...
      TextModel.setSelection(sel.anchorOffset, sel.focusOffset);
    });
    el.addEventListener('keypress', function(e) {
      TextModel.insert(String.fromCharCode(e.which));
      e.preventDefault();
    });
    el.addEventListener('keydown', function(e) {
      var handlers = {};
      handlers[Keys.backspace] = TextModel.deleteBackwards.bind(TextModel);
      handlers[Keys.delete] = TextModel.delete.bind(TextModel);
      handlers[Keys.enter] = function() { TextModel.insert('\n'); };
    ...
    el.addEventListener('input', function(e) {
      if (el.textContent != TextModel.getValue()) {
        console.error('Element content differs from TextModel.');
        console.log(el.textContent);
        console.log(TextModel.getValue());
      }
      TextModel.emit('change');
    });
  }

  return { TextModel: TextModel, connect: connect, setSelection: setSelection };
})({});
